...
class ChunkProcessor:
    ...
    def __init__(self, logger: logging.Logger, config: ChunkingConfig):
        """
    ...
    def chunk_dl2langchain(
        self, chunk: Any, chunker: BaseChunk, metadata_add: dict[str, Any]
    ) -> Document:
        """
    ...
    def chunk_dl2lancedb(
        self, chunk: Any, chunker: BaseChunk, metadata_add: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
    ...
    def save_docling_to_json(self, chunks: list[Any], file_path: str) -> None:
        ...
        with open(file_path, "w", encoding="utf-8") as fp:
            json.dump(
                [chunk.export_json_dict() for chunk in chunks], fp, indent=2, ensure_ascii=False
            )

    def chunk_document(
        self, doc_source: str, metadata_add: dict[str, Any], save_individual_chunks: bool = True
    ) -> list[Any]:
        """
        ...
        try:
            ...
            if save_individual_chunks:
                ...
                chunks_dl_file = os.path.join(
                    self.config.save_location, f"{doc_filename}_chunks.json"
                )

                with open(chunks_dl_file, "w", encoding="utf-8") as fp:
                    json.dump(
                        [chunk.export_json_dict() for chunk in chunks_dl],
                        fp,
                        indent=2,
                        ensure_ascii=False,
                    )
            ...
            self.chunking_errors.append(ChunkingErrorRecord(doc_source, error_msg))
    ...
    def convert_chunks(
        ...
        for i, chunk in enumerate(chunks_dl):
            try:
                ...
                self.logger.error(error_msg)
                self.chunking_errors.append(ChunkingErrorRecord(doc_source, error_msg, i))

        return chunks_converted

    def chunk_all_documents(
        self,
    ) -> tuple[list[Any], list[Document], pd.DataFrame | None]:
        """
        ...
        try:
            ...
            input_paths = io_funcs.get_files(
                self.config.input_dir, self.config.supported_formats, self.logger
            )
            ...
            for i, input_path in enumerate(input_paths):
                try:
                    ...
                    if os.path.getsize(input_path) == 0:
                        ...
                        self.chunking_errors.append(ChunkingErrorRecord(input_path, error_msg))
                    ...
                    chunks_dl = self.chunk_document(
                        input_path, metadata_add, self.config.save_individual_chunks
                    )
                    ...
                    self.logger.error(
                        f"Error chunking document {input_path}: {str(e)}", exc_info=True
                    )
                    self.chunking_errors.append(ChunkingErrorRecord(input_path, str(e)))
            ...
            # Log summary
            self.logger.info(
                f"Chunked {processed_count} documents into {total_chunks_count} segments"
            )
