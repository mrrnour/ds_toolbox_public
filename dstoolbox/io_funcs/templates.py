...
def query_template_reader(query_str: str, replace_dict=None) -> str:
    ...
    query = query_str
    if utils.check_timestamps(
        replace_dict.get("start___date"), replace_dict.get("end___date")
    ):
...
def query_template_run(query_temp_name: str, replace_dict=None,
    ...
    if host == "azure_synapse":
        ...
        if runtime in ("local", "vm_docker"):
            return query_synapse_local(
                query, target_id=host, runtime=runtime, verbose=True,
            )
