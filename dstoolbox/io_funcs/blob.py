...
def spark2blob(df, blob_dict: dict, write_mode: str = "append", runtime: str = "databricks"):
...
def pd2blob(
    ...
    **kwargs
    ...
    if ext == "csv":
        ...
        raise BlobError("pd2blob does not support the legacy .xls format; use xlsx")
    elif ext == "xlsx":
        if append and not overwrite:
            raise BlobError(
                "pd2blob does not append to existing xlsx; " "use xls2blob for multi-sheet writes"
            )
        out = io.BytesIO()
...
def pd2blob_batch(
    outputs: dict, blob_dict: dict, append: bool = True, runtime: str = "databricks", **kwargs
):
    ...
    for out_blob, df in outputs.items():
        try:
            ...
            pd2blob(df, blob_dict=blob_dict, runtime=runtime, append=append, **kwargs)
