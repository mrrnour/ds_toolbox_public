...
def calculate_similarity(df, column, similarity_method):
    ...
    return pd.DataFrame(
        similarity_scores, columns=[column + "_1", column + "_2", "similarity_score"]
    )


def unify_similar_strings(df0, column, similarity_method="similarity_word_dist", threshold=0.8):
    ...
    while i < len(unique_values):
        ...
        while j < len(unique_values):
            similarity = TextSimilarity(
                unique_values[i], unique_values[j], similarity_method
            ).calculate_similarity()
            if (similarity >= threshold) & (unique_values[i] != unique_values[j]):
                replaced_dict[unique_values[i]] = replaced_dict.get(unique_values[i], []) + [
                    (unique_values[j], (df[column] == unique_values[j]).sum())
                ]
                df.loc[df[column] == unique_values[j], column] = unique_values[i]
                unique_values = sorted(
                    df[column].unique(), key=len, reverse=False
    ...
    replaced_df = pd.DataFrame(
        [(k, v[0], v[1]) for k, values in replaced_dict.items() for v in values],
        columns=["Replaced", "Original", "Frequency"],
    )
