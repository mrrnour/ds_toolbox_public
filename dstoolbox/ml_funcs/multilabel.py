...
def split_multilabel_data(df_samples, binarized_tags, random_state=None):
        ...
        train_rows, evalNtest_rows= split_multilabel_data_indices(df_samples.to_numpy(),
                                                                        np.array(binarized_tags_lst.tolist()),
                                                                        test_size=0.30,
                                                                        random_state=None
                                                                )
        df_samples__eval_test=df_samples.iloc[evalNtest_rows]
        eval_rows, test_rows= split_multilabel_data_indices(df_samples__eval_test.to_numpy(),
                                                                np.array(binarized_tags_lst.iloc[evalNtest_rows].tolist()),
                                                                test_size=0.30, random_state=random_state)


        train_idx, eval_idx, test_idx=df_samples.iloc[train_rows].index, df_samples__eval_test.iloc[eval_rows].index, df_samples__eval_test.iloc[test_rows].index
