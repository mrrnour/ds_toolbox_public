...
def _resolve_cv(
    ...
    if sk_fold is None:
        ...
        return zip([range(X.shape[0])], [range(X.shape[0])])
...
def _collect_predictions(
    ...
    if interval_level is not None and hasattr(_unwrap_pipeline(model), 'predict_interval'):
        try:
            ...
            lower, upper = _unwrap_pipeline(model).predict_interval(X_for_interval, level=interval_level)
...
def ml_prediction(ml_model, X, y, sk_fold, X_test=None, y_test=None, callbacks=None, verbose=False, interval_level: float | None = None, include_train_predictions: bool = False):
  """
  ...
  for cv_itr, (train_index, val_index) in enumerate(cv):
    ...
    df_epochs_tmp = _fit_one_fold(
        ml_model, X_train, y_train, X_val, y_val,
    ...
    y_model0 = _collect_predictions(ml_model, X_score, y_score, classifier, interval_level=interval_level)
    ...
    if include_train_predictions:
      try:
        y_train_pred = _collect_predictions(ml_model, X_train, y_train, classifier, interval_level=None)
...
def ml_comparison(ml_models,
  ...
  if show_capabilities:
    derived_names = [
        ...
        or ('-->'.join(s.__class__.__name__ for s in m) if isinstance(m, Pipeline) else m.__class__.__name__)
  ...
  with warnings.catch_warnings():
    ...
    for con, model in enumerate(ml_models):
      ...
      y_model_for_scores = y_model[y_model['split'] == 'val'] if 'split' in y_model.columns else y_model
      cv_results= ml_scores(y_model_for_scores, scores_names,
...
def ml_prediction_nested_cv(ml_model, X, y, outer_fold, inner_fold):
  ...
  for cv_outer, (train_val_index, test_index) in enumerate(outer_fold.split(X, y)):
    ...
    for cv_inner, (train_index, val_index) in enumerate(inner_fold.split(X_train_val, y_train_val)):
