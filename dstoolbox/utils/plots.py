...
def sankey(left, right, value, thershold, utitle, filename):
    ...
    tranactions = tranactions0.groupby(["left", "right"], as_index=False).agg("sum")
    counts = tranactions0.groupby(["left", "right"], as_index=False).agg("count")
    ...
    for i in list(range(left.shape[0])):
        tmpSource = np.where(np.asarray(lbLeft) == np.asarray(left.iloc[i]))[0].tolist()
        source = source + tmpSource

        tmpTarget = np.where(np.asarray(lbRight) == np.asarray(right.iloc[i]))[0].tolist()
    ...
    layout = dict(title=utitle, font=dict(size=10))
...
def wordcloud_graph(txtSeries_df, outputFile):
    ...
    if type(txtSeries_df) == pd.core.series.Series:
        ...
        wordcloud_instance = WordCloud(width=800, height=400).generate_from_frequencies(
            dict(zip(txtSeries_df.iloc[:, 0], txtSeries_df.iloc[:, 1], strict=False))
        )
...
def plot_3d_scatter(udata, uY, xyzLabels, utitle, outPutFile):
    ...
    colors = ["r", "g", "b", "y"]
    for target, color in list(zip(targets, colors, strict=False)):
        indicesToKeep = uY.squeeze() == target
        ax.scatter(
            udata[indicesToKeep, 0],
            udata[indicesToKeep, 1],
            udata[indicesToKeep, 2],
            c=color,
            s=50,
            alpha=0.5,
        )
...
def categorical_color_map(label_col, color_palette=None):
    ...
    if len(domain) > len(color_palette):
        print(
            f"number of available colors({len(color_palette)}) is more than categorizes({len(domain)}), change the palette"
        )
    color_map = dict(zip(domain, color_palette[: len(domain) + 2], strict=False))
    c = label_col.map(color_map)
    return c, color_map


def plotly_group_stack(
    df_plot,
    col2grp,
    col2stack,
    col2c,
    ...
    for shift_name, upattern in zip(df_plot[col2stack].unique(), patterns, strict=False):
    ...
    fig.update_layout(
        barmode="relative", xaxis_title="Date", yaxis_title=col2c, legend_title_text="", title=title
    )
    return fig
...
class DistributionReport:
    ...
    def plot(self):
        ...
        fig.update_layout(
            ...
            width=config.width,
            title=dict(text="Distribution Analysis", x=0.5, font=dict(size=config.title_font_size)),
        ...
        fig.update_xaxes(
            title_text=group_var, row=1, col=1, title_font=dict(size=config.axis_font_size)
        )
        fig.update_xaxes(
            title_text=dependent_var, row=1, col=2, title_font=dict(size=config.axis_font_size)
        )
        fig.update_xaxes(
            title_text=dependent_var, row=2, col=1, title_font=dict(size=config.axis_font_size)
        )
        fig.update_xaxes(
        ...
        fig.update_yaxes(
            title_text=dependent_var, row=1, col=1, title_font=dict(size=config.axis_font_size)
        )
        fig.update_yaxes(
            title_text="Count", row=1, col=2, title_font=dict(size=config.axis_font_size)
        )
        fig.update_yaxes(
        ...
        fig.update_yaxes(
            title_text="Sample Quantiles", row=2, col=2, title_font=dict(size=config.axis_font_size)
        )
...
def hist_plot(df: pd.DataFrame, col: str, quantile_range=None) -> list[float]:
    ...
    y_top = ax.get_ylim()[1]
    for x_loc, q in zip(quan, quantile_range, strict=False):
...
def plot_I_MR(df_I: pd.DataFrame, limits, x_col: str = "TimeStamp", fig=None):
    ...
    for panel_idx, (df_plot, prefix) in enumerate([(df_I, "I_"), (df_MR, "MR_")]):
        ...
        _add_limit_lines(
            fig, limits_sub, x_range, row=panel_idx + 1, is_first_panel=(panel_idx == 0)
        )
