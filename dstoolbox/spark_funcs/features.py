...
def create_rolling_features(
    ...
    w = (
        ...
        .orderBy(F.col(timestamp_column).cast("long"))
...
def create_tumbling_features(
    ...
    df_right = df_right.withColumn("window_start", df_right.window.start).withColumn(
        "window_end", df_right.window.end
    )
    ...
    for column in numeric_cols:
        ...
        df_right = df_right.withColumn(
            f"{column}_{agg_type}", F.col(f"{column}_{agg_type}").cast("float")
        )

    df_merged = asof_join_spark2(
        df_left,
        df_right,
