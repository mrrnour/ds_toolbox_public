...
def save_outputs(outputs_dict_list,
                **kwargs,
              ):
  ...
  **kwargs
  ...
  outputs=outputs_dict_list.items() if isinstance(outputs_dict_list, dict) else outputs_dict_list
...
def update_db_recursively(df_generator_func,
                          output_name,
                          year_range=None,
                          firstDate=None,
                          lastDate=None,
  ...
  **kwargs
  ...
  spark2del_args = list(inspect.signature(io_funcs.spark2delta_table).parameters)
  pd2blob_args = list(inspect.signature(io_funcs.pd2blob).parameters)
  df_generator_func_args = {k: kwargs.pop(k) for k in dict(kwargs) if k not in (spark2del_args+pd2blob_args)}
  ...
  udates=utils.monthly_first_dates(year_range=year_range, 
                          firstDate=firstDate,
                          lastDate=lastDate
                          ###'2020-01-01'
                          )
  ...
  for ii in range(len(udates)-1):     
    try: 
      ...
      ouputs_list=df_generator_func(start_date, end_date,
                                  output_name,
