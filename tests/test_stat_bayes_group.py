...
def test_window_rejects_unequal_lengths_by_default():
    with pytest.raises(ValueError, match="unweighted mean over units"):
        PrePostWindow("2025-12-22", "2026-01-10", *POST)


def test_window_allows_unequal_lengths_when_asked():
...
def test_rope_scales_with_the_baseline():
    assert rope_from_baseline(0.20, 0.10) == pytest.approx((-0.02, 0.02))
    assert rope_from_baseline(0.02, 0.10) == pytest.approx((-0.002, 0.002))


def test_rope_rejects_a_nonpositive_coefficient():
    with pytest.raises(ValueError, match="pct_coef"):
        rope_from_baseline(0.20, 0.0)
...
def test_detects_a_real_lift(lift_effect):
    assert lift_effect.delta_mean > 0
    assert lift_effect.prob_gt_zero > 0.95
    assert lift_effect.decision == "meaningful_positive"
    assert lift_effect.hdi_low < lift_effect.delta_mean < lift_effect.hdi_high
...
def test_finds_nothing_when_nothing_changed():
    ...
    assert effect.decision != "meaningful_positive"
    assert effect.decision != "meaningful_negative"
...
def test_records_the_window_and_the_band(lift_effect):
    ...
    assert lift_effect.rope.rope_high == pytest.approx(
        0.10 * lift_effect.mu_baseline_mean
    )
...
def test_pooled_rate_differs_from_mu(lift_effect):
    ...
    assert lift_effect.pooled_rate_baseline != lift_effect.mu_baseline_mean
...
def test_periods_are_fitted_on_different_seeds(lift_effect):
    ...
    assert not np.array_equal(
        lift_effect.fit_baseline.mu_samples, lift_effect.fit_variant.mu_samples
    )
